...
export type DouyinTrendFetchResult = {
  source: string;
  sourceLabel: string;
  fetchedAt: string;
...
export const getDouyinSearchUrl = (title: string) => `https://www.douyin.com/search/${encodeURIComponent(title)}`;
...
const parsePayload = (raw: string, contentType: string) => {
  if (contentType.includes('application/json') || raw.trim().startsWith('{') || raw.trim().startsWith('[')) {
    try {
      ...
      // Some endpoints return text/html with JSON-like payloads, so continue to HTML parsing.
    }
  }
...
export async function fetchDouyinTrends(): Promise<DouyinTrendFetchResult> {
  const errors: string[] = [];
  ...
  throw new Error(`抖音热搜暂时不可用：${errors.join('；')}`);
}
